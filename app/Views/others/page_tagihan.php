            ...
            success: function(response) {
              if (response.success) {
                ...
                $.each(dataFakultas, function(index, fakultas) {
                  $('#add_fakultas').append('<option value="' + fakultas.ID + '">' + fakultas.deskripsi + '</option>');
                });

                $('#add_fakultas').on('change', function() {
                  const selectedFakultasID = $(this).val();
                  $('#add_prodi').empty().append('<option value="" selected disabled>Pilih Prodi</option>');

                  const defaultProdi = dataProdi.filter(prodi => prodi.fakultas == 0);

                  $.each(defaultProdi, function(index, prodi) {
                    $('#add_prodi').append('<option value="' + prodi.ID + '">' + prodi.deskripsi + '</option>');
                  });

                  const filteredProdi = dataProdi.filter(prodi => prodi.fakultas == selectedFakultasID);

                  $.each(filteredProdi, function(index, prodi) {
                    $('#add_prodi').append('<option value="' + prodi.ID + '">' + prodi.deskripsi + '</option>');
                  });
                });
